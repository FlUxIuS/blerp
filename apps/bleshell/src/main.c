...
static os_membuf_t
	btshell_chr_mem[OS_MEMPOOL_SIZE(BTSHELL_MAX_CHRS, sizeof(struct btshell_chr))];
static struct os_mempool btshell_chr_pool;

static os_membuf_t
	btshell_dsc_mem[OS_MEMPOOL_SIZE(BTSHELL_MAX_DSCS, sizeof(struct btshell_dsc))];
...
static uint64_t start_pairing_time;
...
static void btshell_print_error(char *msg, uint16_t conn_handle,
...
static int btshell_conn_find_idx(uint16_t handle)
{
	int i;
...
static struct btshell_conn *btshell_conn_find(uint16_t handle)
{
	int idx;
...
static struct btshell_svc *btshell_svc_find_prev(struct btshell_conn *conn,
...
static struct btshell_svc *btshell_svc_find(struct btshell_conn *conn,
											uint16_t svc_start_handle,
...
static struct btshell_svc *btshell_svc_find_range(struct btshell_conn *conn,
...
static void btshell_chr_delete(struct btshell_chr *chr)
{
...
static void btshell_svc_delete(struct btshell_svc *svc)
{
...
static struct btshell_svc *btshell_svc_add(uint16_t conn_handle,
...
static struct btshell_chr *btshell_chr_find_prev(const struct btshell_svc *svc,
...
static struct btshell_chr *btshell_chr_find(const struct btshell_svc *svc,
											uint16_t chr_val_handle,
...
static struct btshell_chr *btshell_chr_add(uint16_t conn_handle,
										   uint16_t svc_start_handle,
...
static struct btshell_dsc *btshell_dsc_find_prev(const struct btshell_chr *chr,
												 uint16_t dsc_handle)
{
...
static struct btshell_dsc *btshell_dsc_find(const struct btshell_chr *chr,
											uint16_t dsc_handle,
...
static struct btshell_dsc *btshell_dsc_add(uint16_t conn_handle, uint16_t chr_val_handle,
...
static struct btshell_conn *btshell_conn_add(struct ble_gap_conn_desc *desc)
{
	struct btshell_conn *conn;
...
static void btshell_conn_delete_idx(int idx)
{
	struct btshell_conn *conn;
	struct btshell_svc *svc;
...
static int btshell_on_mtu(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static void btshell_full_disc_complete(int rc)
{
	console_printf("full discovery complete; rc=%d\n", rc);
	btshell_full_disc_prev_chr_val = 0;
}

static void btshell_disc_full_dscs(uint16_t conn_handle)
{
	struct btshell_conn *conn;
	struct btshell_chr *chr;
	struct btshell_svc *svc;
...
static void btshell_disc_full_chrs(uint16_t conn_handle)
{
	struct btshell_conn *conn;
	struct btshell_svc *svc;
	int rc;
...
static int btshell_on_disc_s(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static int btshell_on_disc_c(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static int btshell_on_disc_c_in_s(uint16_t conn_handle,
								  const struct ble_gatt_error *error,
								  const struct ble_gatt_chr *chr, void *arg)
{
...
static int btshell_on_disc_d(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static int btshell_on_read_var(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static int btshell_on_read(uint16_t conn_handle, const struct ble_gatt_error *error,
						   struct ble_gatt_attr *attr, void *arg)
{
	...
	case 0:
		console_printf(
			"characteristic read; conn_handle=%d "
...
static int btshell_on_write(uint16_t conn_handle, const struct ble_gatt_error *error,
							struct ble_gatt_attr *attr, void *arg)
{
	switch (error->status)
	{
	case 0:
		console_printf(
			"characteristic write complete; conn_handle=%d "
			"attr_handle=%d\n",
...
static int btshell_on_write_reliable(uint16_t conn_handle,
									 const struct ble_gatt_error *error,
									 ...
									 void *arg)
{
	...
	case 0:
		console_printf(
			"characteristic write reliable complete; "
			"conn_handle=%d",
			conn_handle);
...
static void btshell_decode_adv_data(const uint8_t *adv_data, uint8_t adv_data_len,
									void *arg)
{
...
#if MYNEWT_VAL(BLE_EXT_ADV)
static void btshell_decode_event_type(struct ble_gap_ext_disc_desc *desc, void *arg)
{
...
common_data:
	console_printf(
		" rssi=%d txpower=%d, pphy=%d, sphy=%d, sid=%d,"
		" periodic_adv_itvl=%u, addr_type=%d addr=",
		desc->rssi, desc->tx_power, desc->prim_phy, desc->sec_phy, desc->sid,
		desc->periodic_adv_itvl, desc->addr.type);
...
static int btshell_restart_adv(struct ble_gap_event *event)
{
	int rc = 0;
#if MYNEWT_VAL(BLE_EXT_ADV)
	uint8_t i;
...
static void handle_periodic_report(struct ble_gap_event *event)
{
	struct psync *psync;
	...
	if (psync->changed || memcmp(psync->data + psync->off, event->periodic_report.data,
...
int btshell_gap_event(struct ble_gap_event *event, void *arg)
{
	...
	case BLE_GAP_EVENT_DISC:
	{
		...
		// 0xf81a2b3f272f;
		// uint8_t pixel[6] = {0x2f, 0x27, 0x3f, 0x2b, 0x1a, 0xf8};
		// // uint8_t macbookpro[6] = {0xc8, 0x1f, 0x1d, 0x74, 0xd0, 0xbc};
		// if (memcmp(event->disc.addr.val, pixel, 6) == 0) {
		//     console_printf("Pixel Found\n");
		//     console_printf("received advertisement; event_type=%d rssi=%d "
		//                "addr_type=%d addr=", event->disc.event_type,
		//                event->disc.rssi, event->disc.addr.type);
		//     print_addr(event->disc.addr.val);
	...
	case BLE_GAP_EVENT_ENC_CHANGE:
		...
		// if (event->enc_change.status == 37) {
		// 	rc = ble_gap_terminate(event->enc_change.conn_handle,
		// BLE_ERR_REM_USER_CONN_TERM); 	assert(rc == 0); 	return 0;
		// }

		if (event->enc_change.status == 66)
		{
			MODLOG_DFLT(INFO, "\nPerforming re-pairing\n");
		...
		console_printf("encrypted=%d, authenticated=%d, bonded=%d\n",
					   desc.sec_state.encrypted, desc.sec_state.authenticated,
					   desc.sec_state.bonded);
		return 0;
	case BLE_GAP_EVENT_PAIRING_COMPLETE:
		console_printf("pairing complete; status=%d\n", event->pairing_complete.status);
		// uint64_t end_paring_time = (os_get_uptime_usec() -
		// start_pairing_time)/1000; printf("\npairing time: %llu ms\n",
		// end_paring_time);
		return 0;
	...
	case BLE_GAP_EVENT_PHY_UPDATE_COMPLETE:
		console_printf(
			"PHY update complete; status=%d, conn_handle=%d "
			" tx_phy=%d, rx_phy=%d\n",
	...
	case BLE_GAP_EVENT_PERIODIC_SYNC:
		...
		{
			console_printf(
				"Periodic Sync Established; sync_handle=%u sid=%u "
				"phy=%u adv_interval=%u ca=%u addr_type=%u addr=",
				event->periodic_sync.sync_handle, event->periodic_sync.sid,
				event->periodic_sync.adv_phy, event->periodic_sync.per_adv_ival,
				event->periodic_sync.adv_clk_accuracy, event->periodic_sync.adv_addr.type);
	...
	case BLE_GAP_EVENT_PERIODIC_SYNC_LOST:
		...
		else
		{
			...
			console_printf(
				"Periodic Sync Lost; sync_handle=%d reason=%d "
				"completed=%u truncated=%u\n",
...
static void btshell_tx_timer_cb(struct os_event *ev)
{
	uint8_t i;
	uint8_t len;
	int32_t timeout;
...
int btshell_disc_all_chrs(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle)
{
	intptr_t svc_start_handle;
	int rc;
...
int btshell_disc_all_dscs(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle)
{
	int rc;
...
int btshell_find_inc_svcs(uint16_t conn_handle, uint16_t start_handle, uint16_t end_handle)
{
	int rc;
...
int btshell_write_no_rsp(uint16_t conn_handle, uint16_t attr_handle, struct os_mbuf *om)
{
	int rc;
...
int btshell_write_long(uint16_t conn_handle, uint16_t attr_handle, uint16_t offset,
					   struct os_mbuf *om)
{
	int rc;

	rc =
		ble_gattc_write_long(conn_handle, attr_handle, offset, om, btshell_on_write, NULL);
	return rc;
}
...
int btshell_ext_conn_initiate(uint8_t own_addr_type, const ble_addr_t *peer_addr,
							  int32_t duration_ms,
				 ...
				 const struct ble_gap_disc_params *disc_params, void *cb_args)
{
	...
	rc =
		ble_gap_disc(own_addr_type, duration_ms, disc_params, btshell_gap_event, cb_args);
	return rc;
}
...
int btshell_sec_start(uint16_t conn_handle)
{
	...
	int rc;
	start_pairing_time = os_get_uptime_usec();
...
static void btshell_on_reset(int reason)
{
	console_printf("Error: Resetting state; reason=%d\n", reason);
}

static void btshell_on_sync(void)
{
...
static void btshell_init_ext_adv_restart(void)
{
#if MYNEWT_VAL(BLE_EXT_ADV)
	int i;
...
int mynewt_main(int argc, char **argv)
{
	...
	rc = os_mempool_init(&btshell_svc_pool, BTSHELL_MAX_SVCS, sizeof(struct btshell_svc),
						 btshell_svc_mem, "btshell_svc_pool");
	assert(rc == 0);

	rc = os_mempool_init(&btshell_chr_pool, BTSHELL_MAX_CHRS, sizeof(struct btshell_chr),
						 btshell_chr_mem, "btshell_chr_pool");
	assert(rc == 0);

	rc = os_mempool_init(&btshell_dsc_pool, BTSHELL_MAX_DSCS, sizeof(struct btshell_dsc),
						 btshell_dsc_mem, "btshell_dsc_pool");
	assert(rc == 0);
	...
	// rc = ble_svc_gap_device_name_set("nimble-bleshell-central");
	// assert(rc == 0);
