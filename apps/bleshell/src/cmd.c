...
static const struct parse_arg_kv_pair cmd_addr_type[] = {
    {"public", BLE_ADDR_PUBLIC}, {"random", BLE_ADDR_RANDOM}, {NULL}};

static int parse_dev_addr(const char *prefix,
                          const struct parse_arg_kv_pair *addr_types,
                          ble_addr_t *addr)
{
  char name[32];
  int rc;
...
static struct parse_arg_kv_pair cmd_ext_adv_phy_opts[] = {
    {"1M", 0x01}, {"2M", 0x02}, {"coded", 0x03}, {NULL}};

static int cmd_advertise_configure(int argc, char **argv)
{
  ...
  params.connectable =
      parse_arg_bool_dflt("connectable", params.connectable, &rc);
  if (rc != 0)
  {
  ...
  params.secondary_phy = parse_arg_kv_dflt(
      "secondary_phy", cmd_ext_adv_phy_opts, params.primary_phy, &rc);
  if (rc != 0)
  {
...
static int cmd_advertise_set_addr(int argc, char **argv)
{
  ble_addr_t addr;
  uint8_t instance;
  int rc;
...
static int cmd_advertise_start(int argc, char **argv)
{
  int max_events;
  uint8_t instance;
  int duration;
...
static int cmd_advertise_stop(int argc, char **argv)
{
  uint8_t instance;
  int rc;
...
static int cmd_advertise_remove(int argc, char **argv)
{
  uint8_t instance;
  int rc;
...
static const struct shell_param advertise_configure_params[] = {
    ...
    {"scan_req_notif",
     "enable Scan Request notification usage: =[0-1], default: 0"},
    {NULL, NULL}};
...
static const struct shell_param advertise_start_params[] = {
    ...
    {"restart",
     "restart advertising after disconnect, usage: =[0-1], default: 0"},
    {NULL, NULL}};
...
// Custom adv set params and start
static int cmd_set_adv_params_and_start()
{
  ...
  rc = ble_gap_adv_start(BLE_ADDR_RANDOM, NULL, BLE_HS_FOREVER, &adv_params,
...
static int cmd_advertise(int argc, char **argv)
{
  ...
  if (argc == 1)
  {
    rc = cmd_set_adv_params_and_start();
    return rc;
  }
  ...
  rc = btshell_adv_start(own_addr_type, peer_addr_param, duration_ms, &params,
                         restart);
  if (rc != 0)
  {
...
static int cmd_disconnect(int argc, char **argv)
{
  ...
  // peer_addr_param->type = MYNEWT_VAL(VICTIM_PRPH_ADDR_TYPE);
...
static int cmd_show_conn(int argc, char **argv)
{
...
static int cmd_show_addr(int argc, char **argv)
{
  uint8_t id_addr[6];
  int rc;
...
static const struct shell_param disconnect_params[] = {
    ...
    {"reason",
     "disconnection reason, usage: =[UINT8], default: 19 (remote user "
     "terminated "
     "connection)"},
    {NULL, NULL}};
...
static int cmd_set_scan_opts(int argc, char **argv)
{
  char *name_filter;
  int rc;
...
static struct parse_arg_kv_pair cmd_scan_ext_types[] = {
    {"none", 0x00}, {"1M", 0x01}, {"coded", 0x02}, {"both", 0x03}, {NULL}};
...
static int cmd_scan(int argc, char **argv)
{
  ...
  if (argc > 1 &&
      (strcmp(argv[1], "cancel") == 0 || strcmp(argv[1], "off") == 0))
  {
  ...
  duration_ms = parse_arg_time_dflt("duration", 10000, 100000, &rc);
  if (rc != 0)
  {
    console_printf("invalid 'duration' parameter\n");
    return rc;
...
static const struct shell_param scan_params[] = {
    ...
    {"filter",
     "usage: =[no_wl|use_wl|no_wl_inita|use_wl_inita], default: no_wl"},
...
static const struct shell_param set_params[] = {
    ...
    {"addr_type",
     "set device address type, usage: =[public|random], default: public"},
...
#if MYNEWT_VAL(BLE_EXT_ADV)
static void update_pattern(uint8_t *buf, int counter)
{
  int i;
...
static int cmd_set_adv_data_or_scan_rsp(int argc, char **argv, bool scan_rsp,
                                        bool periodic)
{
  ...
  static bssnz_t uint8_t public_tgt_addrs[CMD_ADV_DATA_MAX_PUBLIC_TGT_ADDRS]
  ...
  while (1)
  {
    ...
    if (rc == 0)
    {
      ...
      memcpy(public_tgt_addrs[adv_fields.num_public_tgt_addrs], public_tgt_addr,
             BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN);
...
static int cmd_set_adv_data(int argc, char **argv)
{
  return cmd_set_adv_data_or_scan_rsp(argc, argv, false, false);
}

static int cmd_set_scan_rsp(int argc, char **argv)
{
...
static int cmd_set_priv_mode(int argc, char **argv)
{
  ble_addr_t addr;
  uint8_t priv_mode;
  int rc;
...
static const struct shell_param set_priv_mode_params[] = {
    ...
    {"addr_type",
     "set priv mode for device address type, usage: =[public|random], default: "
     "public"},
...
static int cmd_conn_rssi(int argc, char **argv)
{
...
static const struct shell_param conn_rssi_params[] = {
    {"conn", "connection handle parameter, usage: =<UINT16>"}, {NULL, NULL}};
...
static int cmd_conn_update_params(int argc, char **argv)
{
...
static int cmd_conn_datalen(int argc, char **argv)
{
  uint16_t conn_handle;
  uint16_t tx_octets;
  uint16_t tx_time;
...
static const struct shell_param conn_datalen_params[] = {
    ...
    {"octets",
     "Max payload size to include in LL Data PDU, "
     "range=<27-251>, usage: =<UINT16>"},
    {"time",
     "Max number of microseconds the controller should use to tx "
...
static int cmd_keystore_parse_keydata(int argc, char **argv,
                                      union ble_store_key *out, int *obj_type)
{
  int rc;
...
static int cmd_keystore_parse_valuedata(int argc, char **argv, int obj_type,
...
static int cmd_keystore_del(int argc, char **argv)
{
  union ble_store_key key;
  int obj_type;
  int rc;
...
static int cmd_keystore_iterator(int obj_type, union ble_store_value *val,
                                 void *cookie)
{
  switch (obj_type)
  {
...
static const struct shell_param keystore_show_params[] = {
    {"type", "entry type, usage: =<msec|ssec|cccd>"}, {NULL, NULL}};
...
static int cmd_show_oob_sc(int argc, char **argv)
{
...
static int cmd_security_pair(int argc, char **argv)
{
  uint16_t conn_handle;
  int rc;
...
static const struct shell_param security_pair_params[] = {
    {"conn", "connection handle, usage: =<UINT16>"}, {NULL, NULL}};
...
static int cmd_security_unpair(int argc, char **argv)
{
  ble_addr_t peer_addr_param;
  int rc;
  ...
  rc = ble_gap_unpair(&peer_addr_param);
  if (rc != 0)
  {
...
static int cmd_security_start(int argc, char **argv)
{
  uint16_t conn_handle;
  int rc;
...
static int cmd_security_encryption(int argc, char **argv)
{
  uint16_t conn_handle;
  uint16_t ediv;
  uint64_t rand_val;
...
/* Re-pairing extra commands */

#if MYNEWT_VAL(SHELL_CMD_HELP)
static const struct shell_param security_set_fake_authreq_params[] = {
    {"mitm", "usage: =[0-1]"},
    {"bond", "usage: =[0-1]"},
    {"sc", "usage: =[0-1]"},
    {"keypress", "usage: =[0-1]"},
...
static const struct shell_cmd_help security_set_fake_authreq_help = {
    .summary = "set fake authreq flags",
    .usage = NULL,
    .params = security_set_fake_authreq_params,
};
#endif

static int cmd_security_set_fake_authreq(int argc, char **argv)
{
  uint8_t val;
  int rc;
  rc = parse_arg_init(argc - 1, argv + 1);
  ...
  if (rc == 0)
  {
    fake_sec_cfg.sm_mitm = val;
  }

  val = parse_arg_bool_dflt("bond", 1, &rc);
  if (rc == 0)
  {
    fake_sec_cfg.sm_bonding = val;
  }

  val = parse_arg_bool_dflt("sc", 1, &rc);
  if (rc == 0)
  {
    fake_sec_cfg.sm_sc = val;
  }

  val = parse_arg_bool_dflt("keypress", 0, &rc);
  if (rc == 0)
  {
...
#if MYNEWT_VAL(SHELL_CMD_HELP)
static const struct shell_param spoof_address_params[] = {
    {"addr", "usage: =[XX:XX:XX:XX:XX:XX]"},
    {"addr_type", "usage: =[public|random]"},
    {NULL, NULL}};

static const struct shell_cmd_help spoof_address_help = {
    .summary = "set spoofed address",
    .usage = NULL,
    .params = spoof_address_params,
};
#endif

static int cmd_set_spoofed_address(int argc, char **argv)
{
  int rc;
  rc = parse_arg_init(argc - 1, argv + 1);
  if (rc != 0)
  ...
  if (rc != -1)
  {
    ...
    if (rc != 0)
    {
      console_printf("invalid address\n");
      return rc;
    }
  }
...
#if MYNEWT_VAL(SHELL_CMD_HELP)
static const struct shell_param set_spoofed_adv_data_params[] = {
    {"name", "usage: =[string]"},
    {"appearance", "usage: =[UINT16]"},
    {NULL, NULL}};

static const struct shell_cmd_help set_spoofed_adv_data_help = {
    .summary = "set spoofed advertisement data",
    .usage = NULL,
    .params = set_spoofed_adv_data_params,
};
#endif

static int cmd_set_spoofed_adv_data(int argc, char **argv)
{
  struct ble_hs_adv_fields adv_fields;
  int rc;
  ...
  adv_fields.tx_pwr_lvl_is_present = 1;
  adv_fields.tx_pwr_lvl = 4;

  char *name = parse_arg_extract("name");
  ...
  if (adv_fields.name != NULL)
  {
    ...
    if (rc != 0)
    {
      console_printf("error setting device name; rc=%d\n", rc);
      return rc;
    }
  }

  // spoof a mouse by default
  adv_fields.appearance = parse_arg_uint16_dflt("appearance", 962, &rc);
  if (rc == 0)
  {
    adv_fields.appearance_is_present = 1;
    rc = ble_svc_gap_device_appearance_set(adv_fields.appearance);
    if (rc != 0)
    {
      console_printf("error setting device appearance; rc=%d\n", rc);
      return rc;
    }
  }
  else if (rc != ENOENT)
  ...
  // spoofing HID by default
  adv_fields.uuids16 = (ble_uuid16_t[]){BLE_UUID16_INIT(0x1812)};
...
static int cmd_set_repairing_flag_val(int argc, char **argv)
{
  int rc;
  rc = parse_arg_init(argc - 1, argv + 1);
  if (rc != 0)
  ...
  if (rc == 0)
  {
    struct ble_hci_le_custom_cmd_cp cmd;
    cmd.opcode = val;
...
static int cmd_security_set_data(int argc, char **argv)
{
  uint8_t tmp;
  int good;
  int rc;
  ...
  tmp = parse_arg_bool_dflt("oob_flag", 0, &rc);
  if (rc == 0)
  {
  ...
  tmp = parse_arg_bool_dflt("mitm_flag", 0, &rc);
  if (rc == 0)
  {
    good++;
  ...
  tmp = parse_arg_bool_dflt("bonding", 1, &rc);
  if (rc == 0)
  {
    good++;
  ...
  tmp = parse_arg_bool_dflt("sc", 1, &rc);
  if (rc == 0)
  {
    good++;
...
static int cmd_test_tx(int argc, char **argv)
{
  int rc;
  uint16_t conn;
  uint16_t len;
...
static int cmd_phy_set(int argc, char **argv)
{
  uint16_t conn;
...
static int cmd_phy_set_default(int argc, char **argv)
{
  uint8_t tx_phys_mask;
  uint8_t rx_phys_mask;
  int rc;
...
static int cmd_phy_read(int argc, char **argv)
{
  uint16_t conn = 0;
  uint8_t tx_phy;
  uint8_t rx_phy;
...
static int cmd_host_enable(int argc, char **argv)
{
  int rc;
...
static int cmd_host_disable(int argc, char **argv)
{
...
static const struct shell_param gatt_notify_params[] = {
    {"attr", "attribute handle, usage: =<UINT16>"}, {NULL, NULL}};
...
static const struct shell_param gatt_show_params[] = {{NULL, NULL}};
...
static const struct shell_param l2cap_create_server_params[] = {
    {"psm", "usage: =<UINT16>"},
    {"mtu",
     "usage: =<UINT16> not more than BTSHELL_COC_MTU, default BTSHELL_COC_MTU"},
...
static const struct shell_param l2cap_show_coc_params[] = {{NULL, NULL}};
...
#if MYNEWT_VAL(BLE_PERIODIC_ADV)
static int cmd_periodic_configure(int argc, char **argv)
{
...
static int cmd_periodic_set_adv_data(int argc, char **argv)
{
  return cmd_set_adv_data_or_scan_rsp(argc, argv, false, true);
}

static int cmd_periodic_start(int argc, char **argv)
{
  uint8_t instance;
  int rc;
...
static int cmd_periodic_stop(int argc, char **argv)
{
  uint8_t instance;
  int rc;
...
static int cmd_sync_create(int argc, char **argv)
{
...
static int cmd_sync_transfer(int argc, char **argv)
{
...
static int cmd_sync_reporting(int argc, char **argv)
{
  uint16_t sync_handle;
  bool enable;
  int rc;
...
static const struct shell_cmd_help sync_transfer_help = {
    .summary =
        "start periodic sync transfer procedure with specific parameters",
    .usage = NULL,
...
static int cmd_sync_transfer_set_info(int argc, char **argv)
{
  uint16_t service_data;
  uint16_t conn_handle;
  uint8_t instance;
...
static int cmd_sync_transfer_receive(int argc, char **argv)
{
  struct ble_gap_periodic_sync_params params = {0};
  uint16_t conn_handle;
  bool disable;
...
static int cmd_sync_terminate(int argc, char **argv)
{
  uint16_t sync_handle;
  int rc;
...
static const struct shell_param sync_terminate_params[] = {
    {"sync_handle", "usage: =[UINT16], default: 0"}, {NULL, NULL}};
...
static int cmd_sync_stats(int argc, char **argv)
{
  uint16_t sync_handle;
  int rc;
...
#endif
    {
        .sc_cmd = "spoof-address",
        .sc_cmd_func = cmd_set_spoofed_address,
#if MYNEWT_VAL(SHELL_CMD_HELP)
        .help = &spoof_address_help,
#endif
    },
    {
        .sc_cmd = "spoof-authreq",
        .sc_cmd_func = cmd_security_set_fake_authreq,
#if MYNEWT_VAL(SHELL_CMD_HELP)
        .help = &security_set_fake_authreq_help,
#endif
    },
    {
